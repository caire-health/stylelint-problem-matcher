...
var command = __nccwpck_require__(914);
...
async function run() {
    try {
        ...
        const matcherFile = __nccwpck_require__.ab + "problem-matcher.json";
        switch (action) {
            case "add":
                (0,command.issueCommand)("add-matcher", {}, matcherFile);
                break;
            case "remove":
                const fileContents = await (0,promises_namespaceObject.readFile)(matcherFile, { encoding: "utf8" });
                const problemMatcherDocument = JSON.parse(fileContents);
                const problemMatcher = problemMatcherDocument.problemMatcher[0];
                (0,command.issueCommand)("remove-matcher", {
